...
async fn main() -> Result<(), Box<dyn Error + Send + Sync>> {
    ...
    let mut avg_metadata_millis: f64 = 0.;
    for t in tasks {
        let (bytes, metadata_millis) = t.await??;
        bytes_read += bytes.load(std::sync::atomic::Ordering::Relaxed);
        avg_metadata_millis += metadata_millis;
    }

    let avg_metadata_secs = avg_metadata_millis / task_len as f64 / 1000.;
    let elapsed = start_time.elapsed();
    let size = 0.858 * std::cmp::min(count, FILE_PATHS.len()) as f64;
    let secs = elapsed.as_millis() as f64 / 1000.;
    let total_tp = size / secs;
    ...
    let query_secs = secs - avg_metadata_secs;
    let query_tp = bytes_read_gb / query_secs;
    println!("Average Metadata Time: {}", avg_metadata_secs);
    println!("Physical Query Throughput: {:02} GB", query_tp);
    println!("Logical Query Throughput: {:02} GB", size / query_secs);
